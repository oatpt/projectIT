...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  for (int i = 0; i < length; i++) 
     text+=(char) payload[i];
  if(strcmp(topic,"projectit/F5/501/door")==0){
    Serial.printf("hi");
  ...
  if(strcmp(topic,"projectit/F5/501/reboot")==0){
    Serial.printf("hello");
    if(text.compareTo("true")==0)
      ESP.restart();   
  }
...
void setup() {
  ...
  client.subscribe("projectit/F5/501/door");       //กำหนด topic ที่จะ subscribe
  client.subscribe("projectit/F5/501/reboot");
  client.publish("projectit/F5/501/init","true");
}
long timedelay;
void loop() {
  ...
  {
    timedelay= millis();
    client.publish("projectit/F5/501/timeupdate","true");
  }
}
